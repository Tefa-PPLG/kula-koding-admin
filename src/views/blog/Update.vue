...
watch(blog, (blogs) => {
  payload.thumbnail = blogs.thumbnail;
  payload.judul = blogs.judul;
  payload.slug = blogs.slug;
  payload.konten = blogs.konten;
});
...
async function upload() {
  ...
  console.log(payload);
  await UpdateBlog(router.params.id, payload);
}

onMounted(() => {
  ShowBlog(router.params.id);
});
</script>
